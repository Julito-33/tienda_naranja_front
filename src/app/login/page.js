...
export default function PaginaDeLogin() {
  ...
  const [contrasena_es_visible,    set_contrasena_es_visible]    = useState(false);
  ...
  const manejar_envio_del_formulario = async (evento) => {
    ...
    if (resultado.exito) {
      ...
      set_mensaje_de_error('Email o contraseña incorrectos');
    }
  };
        ...
        <form onSubmit={manejar_envio_del_formulario} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                type={contrasena_es_visible ? 'text' : 'password'}
              ...
              <button
                type="button"
                onClick={() => set_contrasena_es_visible(!contrasena_es_visible)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition"
              >
                {contrasena_es_visible ? (
