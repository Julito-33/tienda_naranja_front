...
export default function PaginaDeCheckout() {
  ...
  const { articulos_en_el_carrito, precio_total_en_gs } = useCarritoStore();
  ...
  const [paso_actual,      set_paso_actual]       = useState(1); // 1: envío, 2: pago
  ...
  const confirmar_y_pagar = async () => {
    try {
      ...
      // Paso 1 — crear la orden desde el carrito
      ...
      // Paso 2 — procesar el pago según el método elegido
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className={`flex items-center gap-2 text-sm font-medium ${paso_actual === 1 ? 'text-orange-500' : 'text-gray-400'}`}>
          <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${paso_actual === 1 ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-500'}`}>1</span>
          Datos de envío
        </div>
        <div className="flex-1 h-px bg-gray-200" />
        <div className={`flex items-center gap-2 text-sm font-medium ${paso_actual === 2 ? 'text-orange-500' : 'text-gray-400'}`}>
          <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${paso_actual === 2 ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-500'}`}>2</span>
          Pago
        </div>
      </div>
        ...
        {paso_actual === 1 && (
          <div>
            ...
            <button
              ...
              className="w-full mt-6 bg-orange-500 text-white font-bold py-3 rounded-xl hover:bg-orange-600 transition disabled:opacity-50"
            >
              Continuar al pago →
            </button>
          </div>
          ...
          <div>
            ...
            {metodo_de_pago_elegido === 'transferencia' && (
              <div className="bg-blue-50 rounded-xl p-4 text-sm text-blue-700">
                ...
                <p>Titular: Tienda Naranja S.A.</p>
            ...
            {metodo_de_pago_elegido === 'efectivo' && (
              <div className="bg-green-50 rounded-xl p-4 text-sm text-green-700">
                <p className="font-bold mb-2">Vas a recibir un código para pagar en:</p>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-2 w-full bg-orange-500 text-white font-bold py-3 rounded-xl hover:bg-orange-600 transition disabled:opacity-50"
              >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="border-t border-gray-100 pt-3 mt-3 flex justify-between font-bold text-gray-800">
          <span>Total</span>
          <span className="text-orange-500">Gs. {precio_total_en_gs.toLocaleString('es-PY')}</span>
        </div>
      </div>
    </div>
  );
